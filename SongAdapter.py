...
# %%
class SongLogicAdapter(LogicAdapter):
    ...
    def can_process(self, statement):
       return statement.text.find("我想听") >= 0
    ...
    def process(self, input_statement,additional_response_selection_parameters=None):
        ...
        # tokenize 
        words = ['我想听', '播放', '请放']
        song_name=input_statement.text
        for word in words:
            song_name = song_name.replace(word,"")
        
        
        try:
            song_id = self.songs[ song_name ]
            rst_statement.text = self.__get_song( song_id )
        except Exception as e:
            print( e )
            rst_statement.text = "暂时还没有收录这首歌哦~"
